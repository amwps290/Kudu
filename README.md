...
Kudu 是一款基于 Tauri 2、React 19 和 Rust 构建的轻量级桌面数据库客户端，面向日常查询、结构浏览和轻量数据维护场景。项目重点在于提供稳定、紧凑且一致的桌面数据库工作流体验。
...
- **前端**：React 19、TypeScript、Zustand、Ant Design (antd 5)、Monaco Editor、AG Grid Community
...
│   ├── grid/                AG Grid 结果表与主题
│   ├── layout/              应用标题栏、状态栏
│   ├── right-panel/         右侧信息面板（cell viewer / output / 对象信息）
│   ├── search/              全局搜索
│   ├── settings/            设置面板
│   └── tools/               查询构建器、数据对比、ER 图
├── hooks/                   业务 hooks（标签管理、SQL 执行、Monaco、会话生命周期等）
├── i18n/                    i18next 初始化
├── locales/                 i18n 翻译文件（zh-CN、en-US）
├── services/                SQL/Redis 自动补全服务
├── stores/                  Zustand 状态管理（app、connection、workspace、rightPanel）
├── types/                   TypeScript 类型定义
├── utils/                   工具函数（剪贴板、错误处理、数据库支持配置等）
└── views/                   主视图（HomeView 工作台）
