...
use tokio_opengauss::{CancelToken, Client, Config, NoTls};
use tracing::{debug, error, info, instrument, warn};

use super::constants::PG_DEFAULT_SCHEMA;
use super::sql_helpers::{build_where_clause, ParamStyle};
use super::traits::*;
use crate::utils::sql_sanitize::{escape_opengauss_id, escape_string_literal};

/// openGauss 驱动状态容器 - 用于内部互斥
struct ActiveOpenGaussQuery {
    query_id: u64,
    cancel_token: CancelToken,
    backend_pid: i64,
}

struct OpenGaussState {
    client: Option<Arc<Client>>,
    config: Option<ConnectionConfig>,
    active_query: Option<ActiveOpenGaussQuery>,
    notices: Arc<StdMutex<Vec<DbMessage>>>,
}

/// openGauss 数据库驱动 - 基于 tokio-postgres 的底层实现 (具备内部并发能力)
pub struct OpenGaussDatabase {
    state: Mutex<OpenGaussState>,
}

impl OpenGaussDatabase {
    pub fn new() -> Self {
        Self {
            state: Mutex::new(OpenGaussState {
                client: None,
                config: None,
                active_query: None,
    ...
    fn normalize_simple_query_sql(sql: &str) -> String {
    ...
    async fn get_client_arc(&self) -> DbResult<Arc<Client>> {
        ...
        state
            ...
            .as_ref()
            .cloned()
            .ok_or(DbError::not_connected())
    }

    fn log_sql_error(context: &str, sql: &str, error: &tokio_opengauss::Error) {
        error!(
            context = context,
            sql = %sql.replace('\n', " "),
            error = %Self::format_opengauss_error(&error),
            "SQL 执行失败"
        );
    }

    fn into_query_error(context: &str, sql: &str, error: tokio_opengauss::Error) -> DbError {
        Self::log_sql_error(context, sql, &error);
        DbError::QueryFailed(Self::format_opengauss_error(&error))
    }
    ...
    async fn query_logged(
        ...
        context: &str,
        sql: &str,
        params: &[&(dyn tokio_opengauss::types::ToSql + Sync)],
    ) -> DbResult<Vec<tokio_opengauss::Row>> {
        client
            .query(sql, params)
            .await
            .map_err(|e| Self::into_query_error(context, sql, e))
    }

    async fn query_one_logged(
        client: &Client,
        context: &str,
        sql: &str,
        params: &[&(dyn tokio_opengauss::types::ToSql + Sync)],
    ) -> DbResult<tokio_opengauss::Row> {
        client
    ...
    async fn batch_execute_logged(client: &Client, context: &str, sql: &str) -> DbResult<()> {
        client
            .batch_execute(sql)
            .await
            .map_err(|e| Self::into_query_error(context, sql, e))
    ...
    async fn simple_query_logged(
        client: &Client,
        context: &str,
        sql: &str,
    ) -> DbResult<Vec<tokio_opengauss::SimpleQueryMessage>> {
        client
            .simple_query(sql)
            .await
            .map_err(|e| Self::into_query_error(context, sql, e))
    ...
    async fn get_aggregate_definition(
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 聚合函数定义 SQL");
        let rows = match client.query(sql.as_str(), &[]).await {
            Ok(rows) => {
                info!(matched = rows.len(), "openGauss 聚合函数定义查询完成");
                rows
            }
            Err(error) => {
                Self::log_sql_error("openGauss 聚合函数定义 SQL", &sql, &error);
                let formatted = Self::format_opengauss_error(&error);
                error!(
                    schema = %schema_name,
                    name = %name,
    ...
    async fn create_client(
        ...
        client_config
            ...
            .connect_timeout(std::time::Duration::from_secs(timeout_secs.into()));
        ...
        let (client, connection) = client_config
            .connect(NoTls)
            .await
            .map_err(|e| DbError::ConnectionFailed(Self::format_opengauss_error(&e)))?;
        ...
        T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
        ...
        loop {
            match poll_fn(|cx| conn.as_mut().poll_message(cx)).await {
                Some(Ok(msg)) => {
                    if let tokio_opengauss::AsyncMessage::Notice(notice) = msg {
                        let severity = notice.severity().to_string();
                        let text = notice.message().to_string();
                        debug!(severity = %severity, text = %text, "openGauss 通知");
    ...
    async fn set_active_query(&self, query_id: u64, cancel_token: CancelToken, backend_pid: i64) {
        let mut state = self.state.lock().await;
        state.active_query = Some(ActiveOpenGaussQuery {
            query_id,
            cancel_token,
            backend_pid,
        });
    ...
    async fn get_cancel_target(
        &self,
        query_id: u64,
    ) -> DbResult<Option<(ConnectionConfig, CancelToken, i64)>> {
        let state = self.state.lock().await;
        ...
        Ok(Some((
            config,
            active_query.cancel_token.clone(),
            active_query.backend_pid,
        )))
    }

    /// 将 serde_json::Value 转换为 tokio-postgres 的参数类型
    fn json_to_opengauss_param(
        value: &serde_json::Value,
    ) -> Box<dyn tokio_opengauss::types::ToSql + Sync + Send> {
        match value {
    ...
    fn format_opengauss_error(error: &tokio_opengauss::Error) -> String {
...
#[async_trait]
impl DatabaseOperations for OpenGaussDatabase {
    async fn test_connection(&self, config: &ConnectionConfig) -> DbResult<bool> {
        ...
        Self::query_logged(&client, "openGauss 测试连接 SQL", "SELECT 1", &[]).await?;
        Ok(true)
    }
    ...
    async fn check_health(&self) -> DbResult<bool> {
        ...
        Self::query_logged(&client, "openGauss 健康检查 SQL", "SELECT 1", &[]).await?;
        Ok(true)
    }
    ...
    async fn switch_database(&self, database: &str) -> DbResult<()> {
        ...
        info!(new_db = %database, "openGauss 正在物理切换数据库连接...");
    ...
    async fn execute_query(
        ...
        if let Some(query_id) = query_id {
            let backend_pid: i64 = Self::query_one_logged(
                &client,
                "openGauss 查询 backend pid SQL",
                "SELECT pg_backend_pid()",
                &[],
            )
            .await?
            .get(0);
            self.set_active_query(query_id, client.cancel_token(), backend_pid)
                .await;
        }

        let normalized_sql = Self::normalize_simple_query_sql(sql);
        debug!(
            sql = %normalized_sql.replace('\n', " "),
            original_sql = %sql.replace('\n', " "),
            "执行查询"
        );
        ...
        let result = async {
            ...
            let messages = Self::simple_query_logged(
                &client,
                "openGauss simple query SQL",
                &normalized_sql,
            )
            .await?;
            ...
            for msg in messages {
                match msg {
                    tokio_opengauss::SimpleQueryMessage::Row(row) => {
                    ...
                    tokio_opengauss::SimpleQueryMessage::CommandComplete(count) => {
    ...
    async fn get_databases(&self) -> DbResult<Vec<DatabaseInfo>> {
        ...
        let sql = "
            ...
            ORDER BY d.datname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取数据库列表 SQL", sql, &[]).await?;
        Ok(rows
            .into_iter()
            .map(|r| DatabaseInfo {
                name: r.get(0),
    ...
    async fn get_schemas(&self, _db: Option<&str>) -> DbResult<Vec<SchemaInfo>> {
        ...
        let sql = "
            ...
            ORDER BY n.nspname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取 schema 列表 SQL", sql, &[]).await?;
        Ok(rows
            .into_iter()
            .map(|r| SchemaInfo {
                oid: r.try_get(0).ok(),
    ...
    async fn get_tables(&self, _db: Option<&str>) -> DbResult<Vec<TableInfo>> {
        ...
        let sql = "
            SELECT
                ...
                COALESCE((SELECT option_value FROM pg_options_to_table(c.reloptions) WHERE option_name = 'fillfactor' LIMIT 1), '100')::text AS fillfactor,
                NULL::text AS partition_key,
                CASE WHEN pc.relkind = 'p' THEN pn.nspname::text END AS parent_schema,
                CASE WHEN pc.relkind = 'p' THEN pc.relname::text END AS parent_name,
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取表列表 SQL", sql, &[]).await?;
        ...
        ";
        let partition_rows = Self::query_logged(&client, "openGauss 获取分区列表 SQL", partitions_sql, &[]).await?;
        ...
        Ok(rows
            ...
            .map(|r| {
                ...
                TableInfo {
                    ...
                    name: r.get(2),
                    schema: Some(r.get(1)),
                    table_type: if relkind == "p" {
                        "PARTITIONED TABLE".into()
                    } else if partition_parent.is_some() {
                        "PARTITION".into()
                    } else {
                        "TABLE".into()
                    },
    ...
    async fn get_views(&self, _db: Option<&str>) -> DbResult<Vec<TableInfo>> {
        ...
        let sql = "
            ...
            ORDER BY n.nspname, c.relname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取视图列表 SQL", sql, &[]).await?;
        Ok(rows
            .into_iter()
            .map(|r| TableInfo {
                oid: None,
    ...
    async fn get_materialized_views(
        ...
        let sql = "
            SELECT n.nspname, c.relname, pg_catalog.pg_get_userbyid(c.relowner)::text, COALESCE(ts.spcname, 'pg_default')::text, obj_description(c.oid)::text, pg_total_relation_size(c.oid)::int8, pg_total_relation_size(c.oid)::float8 / 1024 / 1024
            FROM pg_class c
            JOIN pg_namespace n ON n.oid = c.relnamespace
            LEFT JOIN pg_tablespace ts ON ts.oid = c.reltablespace
            WHERE c.relkind = 'm'
            ...
            ORDER BY n.nspname, c.relname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取物化视图列表 SQL", sql, &[&schema_name]).await?;
        Ok(rows
            .into_iter()
            .map(|r| TableInfo {
                oid: None,
    ...
    async fn get_table_structure(
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取表结构 SQL", sql, &[&table, &schema_name]).await?;
        ...
        let pk_rows = client
            .query(pk_sql, &[&format!("{}.{}", schema_name, table)])
            .await
            .unwrap_or_default();
        ...
        Ok(rows
            ...
            .map(|r| {
                ...
                ColumnInfo {
                    ...
                    comment: None,
                    character_maximum_length: if max_len > 0 {
                        Some(max_len as i64)
                    } else {
                        None
                    },
    ...
    async fn update_data(
        ...
        let wc = build_where_clause(
            &where_conditions,
            escape_opengauss_id,
            ParamStyle::DollarNumber(1),
        );

        let sql = format!(
            "UPDATE {}.{} SET {} = $1 WHERE {}",
            escape_opengauss_id(schema_name),
            escape_opengauss_id(table),
            escape_opengauss_id(column),
            wc.sql
        );

        debug!(sql = %sql, "执行 openGauss 参数化更新");

        // 参数放独立作用域以确保在 await 点之前 drop
        let result = {
            let set_param: Box<dyn tokio_opengauss::types::ToSql + Sync + Send> = match &value {
                Some(v) => Box::new(v.clone()),
                None => Box::new(None::<String>),
            };
            let mut owned: Vec<Box<dyn tokio_opengauss::types::ToSql + Sync + Send>> =
                vec![set_param];
            ...
            let mut refs: Vec<&(dyn tokio_opengauss::types::ToSql + Sync)> =
                Vec::with_capacity(owned.len());
            for p in &owned {
                refs.push(p.as_ref());
            }
            Self::execute_logged(&client, "openGauss 参数化更新 SQL", sql.as_str(), &refs).await
        };
        result?;
        Ok(())
    }
    ...
    async fn insert_data(
        ...
        let columns = entries
            .iter()
            .map(|(name, _)| escape_opengauss_id(name))
            .collect::<Vec<_>>()
            .join(", ");
        ...
        debug!(sql = %sql, "执行 openGauss 参数化插入");

        let result = {
            let owned: Vec<Box<dyn tokio_opengauss::types::ToSql + Sync + Send>> = entries
                .iter()
                .map(|(_, value)| Self::json_to_opengauss_param(value))
                .collect();
            let mut refs: Vec<&(dyn tokio_opengauss::types::ToSql + Sync)> =
                Vec::with_capacity(owned.len());
            for p in &owned {
            ...
            Self::execute_logged(&client, "openGauss 参数化插入 SQL", sql.as_str(), &refs).await
        };
        result?;
        Ok(())
    }
    ...
    async fn delete_data(
        ...
        debug!(sql = %sql, "执行 openGauss 参数化删除");

        let result = {
            let mut owned: Vec<Box<dyn tokio_opengauss::types::ToSql + Sync + Send>> = Vec::new();
            for pv in &wc.param_values {
                match pv {
                    Some(s) => owned.push(Box::new(s.clone())),
                    None => owned.push(Box::new(None::<String>)),
            ...
            Self::execute_logged(&client, "openGauss 参数化删除 SQL", sql.as_str(), &refs).await
        };
        result?;
        Ok(())
    }
    ...
    async fn truncate_table(&self, table: &str, schema: Option<&str>) -> DbResult<()> {
        ...
        debug!(sql = %sql, "执行 openGauss 清空表");
        Self::batch_execute_logged(&client, "openGauss 清空表 SQL", &sql).await?;
        Ok(())
    }
    ...
    async fn get_indexes(&self, table: &str, schema: Option<&str>) -> DbResult<Vec<IndexInfo>> {
        ...
        let sql = "
            ...
            ORDER BY i.relname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取索引列表 SQL", sql, &[&table, &schema_name]).await?;
    ...
    async fn get_schema_indexes(
        ...
        let sql = "
            ...
            ORDER BY i.relname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取 schema 索引 SQL", sql, &[&schema_name]).await?;
        let indexes: Vec<IndexInfo> = rows
            ...
            .collect();
        debug!(count = indexes.len(), sc = %schema_name, "已获取 openGauss 索引");
        Ok(indexes)
    }
    ...
    async fn get_foreign_keys(
        ...
        let rows = Self::query_logged(&client, "openGauss 获取外键列表 SQL", sql, &[&table, &schema_name]).await?;
    ...
    async fn get_triggers(
        ...
        let sql = "
            ...
            ORDER BY tg.tgname
        ";
        let rows = Self::query_logged(&client, "openGauss 获取触发器列表 SQL", sql, &[&table, &schema_name]).await?;
        Ok(rows
            .into_iter()
            .map(|r| {
    ...
    async fn get_table_constraints(
        ...
        schema: Option<&str>,
        _database: Option<&str>,
    ) -> DbResult<Vec<TableConstraintInfo>> {
        let client = self.get_client_arc().await?;
        let schema_name = schema.unwrap_or(PG_DEFAULT_SCHEMA);
        let sql = "
            SELECT
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取约束列表 SQL", sql, &[&table, &schema_name]).await?;
        Ok(rows
            .into_iter()
            .map(|r| TableConstraintInfo {
                name: r.get(0),
    ...
    async fn get_rules(
        ...
        let sql = "
            ...
            ORDER BY r.rulename
        ";
        let rows = Self::query_logged(&client, "openGauss 获取规则列表 SQL", sql, &[&table, &schema_name]).await?;
        Ok(rows
            .into_iter()
            .map(|r| RuleInfo {
                name: r.get(0),
    ...
    async fn alter_table(
            ...
            match change {
                TableChange::AddColumn(col) => {
                    ...
                    if let Some(ref d) = col.default_value {
                        part.push_str(&format!(" DEFAULT {}", escape_string_literal(d)));
                    }
                    sql_parts.push(part);
                }
                TableChange::ModifyColumn {
                    ...
                    if old_name != &new_column.name {
                        sql_parts.push(format!(
                            "RENAME COLUMN {} TO {}",
                            escape_opengauss_id(old_name),
                            escape_opengauss_id(&new_column.name)
                        ));
                    }
                    sql_parts.push(format!(
                        "ALTER COLUMN {} TYPE {}",
                ...
                TableChange::ReorderColumn { .. } => {
                    return Err(DbError::Other("openGauss 暂不支持调整字段顺序".into()));
                }
                TableChange::DropColumn(name) => {
                    sql_parts.push(format!("DROP COLUMN {}", escape_opengauss_id(name)));
                }
                TableChange::AddIndex(idx) => {
                    let cols = idx
                        .columns
                        .iter()
                        .map(|c| escape_opengauss_id(c))
                        .collect::<Vec<_>>()
                        .join(", ");
                    ...
                    sql_parts.push(format!(
                        "ADD {}INDEX {} ({})",
                        unique,
                        escape_opengauss_id(&idx.name),
                        cols
                    ));
                }
                TableChange::DropIndex(name) => {
                    // openGauss DROP INDEX 是独立命令，不能放在 ALTER TABLE 中
                    debug!(name = %name, "准备删除 openGauss 索引");
                }
                TableChange::AddForeignKey(fk) => {
                    sql_parts.push(format!(
                        "ADD CONSTRAINT {} FOREIGN KEY ({}) REFERENCES {}.{} ({}) ON UPDATE {} ON DELETE {}",
                        escape_opengauss_id(&fk.name), escape_opengauss_id(&fk.column_name), escape_opengauss_id(schema_name), escape_opengauss_id(&fk.referenced_table_name), escape_opengauss_id(&fk.referenced_column_name),
                        fk.update_rule.as_deref().unwrap_or("NO ACTION"),
                        fk.delete_rule.as_deref().unwrap_or("NO ACTION")
                    ));
                }
                TableChange::DropForeignKey(name) => {
                    sql_parts.push(format!("DROP CONSTRAINT {}", escape_opengauss_id(name)));
                }
            }
        }
        ...
        if !sql_parts.is_empty() {
            ...
            debug!(sql = %alter_sql, "执行 openGauss ALTER TABLE");
            Self::batch_execute_logged(&client, "openGauss ALTER TABLE SQL", &alter_sql).await?;
        }
        ...
        for change in &changes {
            if let TableChange::DropIndex(name) = change {
                let drop_idx_sql = format!(
                    "DROP INDEX {}.{}",
                    escape_opengauss_id(schema_name),
                    escape_opengauss_id(name)
                );
                debug!(sql = %drop_idx_sql, "执行 openGauss DROP INDEX");
                Self::batch_execute_logged(&client, "openGauss DROP INDEX SQL", &drop_idx_sql).await?;
            }
        }
    ...
    async fn get_table_ddl(&self, table: &str, schema: Option<&str>) -> DbResult<String> {
        let schema_name = schema.unwrap_or(PG_DEFAULT_SCHEMA);
        let columns = self
            .get_table_structure(table, Some(schema_name), None)
            .await?;
        ...
        let view_rows = client
            .query(view_sql, &[&schema_name, &table])
            .await
            .unwrap_or_default();
        ...
        for col in columns {
            let mut def = format!("  {} {}", escape_opengauss_id(&col.name), col.data_type);
            if !col.nullable {
                def.push_str(" NOT NULL");
            }
            if let Some(ref d) = col.default_value {
                def.push_str(&format!(" DEFAULT {}", escape_string_literal(d)));
            }
            col_defs.push(def);
            if col.is_primary_key {
                pks.push(escape_opengauss_id(&col.name));
            }
        }
    ...
    async fn get_view_definition(&self, view: &str, schema: Option<&str>) -> DbResult<String> {
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取视图定义 SQL", sql, &[&schema_name, &view]).await?;
    ...
    async fn get_index_definition(
        ...
        let sql = "
            ...
            WHERE n.nspname = $1
              AND i.relname = $2
        ";
        let rows = Self::query_logged(&client, "openGauss 获取索引定义 SQL", sql, &[&schema_name, &index]).await?;
        if let Some(row) = rows.first() {
            let definition: String = row.get(0);
    ...
    async fn get_routine_definition(
        ...
        if routine_type == "aggregate" {
            return self
                .get_aggregate_definition(&client, schema_name, name, identity_arguments, oid)
                .await;
        }
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 例程定义 SQL");
        let rows = match client.query(sql.as_str(), &[]).await {
            Ok(rows) => {
                info!(matched = rows.len(), "openGauss 例程定义查询完成");
                rows
            }
            Err(error) => {
                let formatted = Self::format_opengauss_error(&error);
    ...
    async fn get_functions(
        ...
        Ok(rows
            ...
            .map(|r| FunctionInfo {
                ...
                name: r.get(1),
                schema: Some(r.get(2)),
                return_type: r.try_get(3).ok(),
                arguments: r.try_get(4).ok(),
                identity_arguments: r.try_get(5).ok(),
                language: r.try_get(6).ok(),
                function_type: "function".into(),
                volatility: r.try_get(7).ok(),
                security_definer: r.try_get(8).ok(),
                parallel: r.try_get(9).ok(),
                is_strict: r.try_get(10).ok(),
                leakproof: r.try_get(11).ok(),
                estimated_cost: r.try_get(12).ok(),
                estimated_rows: r.try_get(13).ok(),
                comment: r.try_get(14).ok(),
            })
            .collect())
    }
    ...
    async fn get_aggregate_functions(
        ...
        let client = self.get_client_arc().await?;
        let schema_name = schema.unwrap_or(PG_DEFAULT_SCHEMA);
        let sql = "SELECT p.oid::int8, p.proname, n.nspname, pg_catalog.pg_get_function_result(p.oid), pg_catalog.pg_get_function_arguments(p.oid), pg_catalog.pg_get_function_identity_arguments(p.oid), CASE p.provolatile WHEN 'i' THEN 'IMMUTABLE' WHEN 's' THEN 'STABLE' ELSE 'VOLATILE' END, p.prosecdef, NULL::text AS parallel, p.proisstrict, p.proleakproof, p.procost::float8, p.prorows::float8, obj_description(p.oid, 'pg_proc') FROM pg_aggregate a JOIN pg_proc p ON p.oid = a.aggfnoid JOIN pg_namespace n ON n.oid = p.pronamespace WHERE n.nspname = $1 ORDER BY p.proname, p.oid";
        let rows = Self::query_logged(&client, "openGauss 获取聚合函数列表 SQL", sql, &[&schema_name]).await?;
        Ok(rows
            .into_iter()
            .map(|r| FunctionInfo {
                oid: r.try_get(0).ok(),
    ...
    async fn get_procedures(
        ...
        debug!(sc = %schema_name, "正在查询 openGauss 存储过程...");
        let rows = Self::query_logged(&client, "openGauss 获取存储过程列表 SQL", sql, &[&schema_name]).await?;
        debug!(count = rows.len(), "已获取 openGauss 存储过程列表");
    ...
    async fn get_extensions(&self, _database: Option<&str>) -> DbResult<Vec<ExtensionInfo>> {
        let client = self.get_client_arc().await?;
        let sql = "SELECT e.oid::int8, extname::text, extversion::text, n.nspname::text, e.extrelocatable, obj_description(e.oid, 'pg_extension')::text FROM pg_extension e JOIN pg_namespace n ON n.oid = e.extnamespace ORDER BY extname";

        debug!("正在查询 openGauss 扩展...");
        let rows = Self::query_logged(&client, "openGauss 获取扩展列表 SQL", sql, &[]).await?;
        ...
        Ok(rows
            ...
            .map(|r| ExtensionInfo {
                ...
                version: r.get(2),
                schema: Some(r.get(3)),
                relocatable: r.try_get(4).ok(),
                comment: r.try_get(5).ok(),
            })
            .collect())
    }
    ...
    async fn get_available_extensions(&self, _database: Option<&str>) -> DbResult<Vec<String>> {
        ...
        let sql = "
            ...
            ORDER BY name
        ";
        let rows = Self::query_logged(&client, "openGauss 获取可用扩展列表 SQL", sql, &[]).await?;
        Ok(rows
            .into_iter()
    ...
    async fn get_sequences(
        ...
        let sql = "
            SELECT
                ...
                s.maximum_value::int8,
                s.increment::int8,
                NULL::int8 AS cache_size,
                (s.cycle_option = 'YES') AS cycle,
                obj_description(c.oid, 'pg_class')
            FROM information_schema.sequences s
            LEFT JOIN pg_namespace n ON n.nspname = s.sequence_schema
            LEFT JOIN pg_class c ON c.relnamespace = n.oid AND c.relname = s.sequence_name AND c.relkind = 'S'
            WHERE s.sequence_schema = $1
            ORDER BY s.sequence_name
        ";
        let rows = Self::query_logged(&client, "openGauss 获取序列列表 SQL", sql, &[&schema_name]).await?;
        Ok(rows
            .into_iter()
            .map(|r| SequenceInfo {
                oid: r.try_get(0).ok(),
    ...
    async fn get_enum_types(
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取枚举类型列表 SQL", sql, &[&schema_name]).await?;
        ...
        for row in rows {
            ...
            if let Some(existing) = enums.last_mut().filter(|item| {
                item.name == name && item.schema.as_deref() == Some(row_schema.as_str())
            }) {
    ...
    async fn get_enum_definition(
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 枚举类型定义 SQL");
        let rows = client.query(sql.as_str(), &[]).await.map_err(|e| {
            Self::log_sql_error("openGauss 枚举类型定义 SQL", &sql, &e);
            DbError::QueryFailed(Self::format_opengauss_error(&e))
        })?;
        if rows.is_empty() {
        ...
        Ok(format!(
            "CREATE TYPE {}.{} AS ENUM ({});",
            escape_opengauss_id(&enum_schema),
            escape_opengauss_id(&enum_name),
            labels,
        ))
    }

    async fn get_domain_types(
        &self,
        _database: Option<&str>,
        schema: Option<&str>,
    ) -> DbResult<Vec<DomainTypeInfo>> {
        let client = self.get_client_arc().await?;
        let schema_name = schema.unwrap_or(PG_DEFAULT_SCHEMA);
        let sql = "
            SELECT
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取域类型列表 SQL", sql, &[&schema_name]).await?;
        ...
        for row in rows {
            ...
            if let Some(existing) = domains.last_mut().filter(|item| {
                item.name == name && item.schema.as_deref() == Some(row_schema.as_str())
            }) {
    ...
    async fn get_domain_definition(
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 域类型定义 SQL");
        let rows = client.query(sql.as_str(), &[]).await.map_err(|e| {
            Self::log_sql_error("openGauss 域类型定义 SQL", &sql, &e);
            DbError::QueryFailed(Self::format_opengauss_error(&e))
        })?;
        if rows.is_empty() {
        ...
        let mut parts = vec![format!(
            "CREATE DOMAIN {}.{} AS {}",
            escape_opengauss_id(&domain_schema),
            escape_opengauss_id(&domain_name),
            base_type
        )];
        ...
        for row in rows {
            ...
            if let Some(def) = constraint_def {
                if let Some(name) = constraint_name {
                    parts.push(format!("CONSTRAINT {} {}", escape_opengauss_id(&name), def));
                } else {
                    parts.push(def);
                }
            }
    ...
    async fn get_composite_types(
        ...
        _database: Option<&str>,
        schema: Option<&str>,
    ) -> DbResult<Vec<CompositeTypeInfo>> {
        let client = self.get_client_arc().await?;
        let schema_name = schema.unwrap_or(PG_DEFAULT_SCHEMA);
        let sql = "
            SELECT
        ...
        ";
        let rows = Self::query_logged(&client, "openGauss 获取复合类型列表 SQL", sql, &[&schema_name]).await?;
        ...
        for row in rows {
            ...
            if let Some(existing) = composites.last_mut().filter(|item| {
                item.name == name && item.schema.as_deref() == Some(row_schema.as_str())
            }) {
                if let (Some(field_name), Some(data_type)) = (field_name, field_type) {
                    existing.fields.push(CompositeFieldInfo {
                        name: field_name,
                        data_type,
                    });
                }
    ...
    async fn get_composite_definition(
        &self,
        name: &str,
        schema: Option<&str>,
        _database: Option<&str>,
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 复合类型定义 SQL");
        let rows = client.query(sql.as_str(), &[]).await.map_err(|e| {
            Self::log_sql_error("openGauss 复合类型定义 SQL", &sql, &e);
            DbError::QueryFailed(Self::format_opengauss_error(&e))
        })?;
        if rows.is_empty() {
        ...
        let fields = rows
            ...
            .filter_map(|row| {
                ...
                match (field_name, field_type) {
                    (Some(field_name), Some(field_type)) => Some(format!(
                        "    {} {}",
                        escape_opengauss_id(&field_name),
                        field_type
                    )),
                    _ => None,
                }
        ...
        Ok(format!(
            "CREATE TYPE {}.{} AS (\n{}\n);",
            escape_opengauss_id(&composite_schema),
            escape_opengauss_id(&composite_name),
            fields.join(",\n"),
        ))
    }
    ...
    async fn get_sequence_definition(
        ...
        let sql = format!(
            ...
            ORDER BY c.oid NULLS LAST, s.sequence_name
            LIMIT 1
            ",
            schema = escape_string_literal(schema_name),
            oid_condition = oid_condition,
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 序列定义 SQL");
        let rows = client.query(sql.as_str(), &[]).await.map_err(|e| {
            Self::log_sql_error("openGauss 序列定义 SQL", &sql, &e);
            DbError::QueryFailed(Self::format_opengauss_error(&e))
        })?;
        if let Some(row) = rows.first() {
            ...
            let definition = format!(
                "CREATE SEQUENCE {}.{}\n    AS {}\n    START WITH {}\n    INCREMENT BY {}\n    MINVALUE {}\n    MAXVALUE {}\n    CACHE {}\n    {};",
                escape_opengauss_id(&sequence_schema),
                escape_opengauss_id(&sequence_name),
                data_type,
                start_value,
                increment_by,
                min_value,
    ...
    async fn get_sequence_state(
        ...
        _database: Option<&str>,
        oid: Option<i64>,
    ) -> DbResult<SequenceStateInfo> {
        let client = self.get_client_arc().await?;
        let schema_name = schema.unwrap_or(PG_DEFAULT_SCHEMA);
        let oid_condition = oid
            .map(|value| format!("c.oid = {}::oid", value))
            .unwrap_or_else(|| "FALSE".to_string());
        let sequence_ref = format!(
            "{}.{}",
            escape_opengauss_id(schema_name),
            escape_opengauss_id(name)
        );
        let sql = format!(
            ...
            SELECT
                ...
                s.increment::int8,
                CASE
                    WHEN seq.is_called THEN seq.last_value + s.increment::int8
                    ELSE seq.last_value
                END AS next_value
            FROM information_schema.sequences s
            LEFT JOIN pg_namespace n ON n.nspname = s.sequence_schema
        ...
        info!(sql = %sql.replace('\n', " "), "执行 openGauss 序列状态 SQL");
        let rows = client.query(sql.as_str(), &[]).await.map_err(|e| {
            Self::log_sql_error("openGauss 序列状态 SQL", &sql, &e);
            DbError::QueryFailed(Self::format_opengauss_error(&e))
        })?;
        if let Some(row) = rows.first() {
    ...
    async fn cancel_query(&self, query_id: u64) -> DbResult<bool> {
        let Some((config, cancel_token, backend_pid)) = self.get_cancel_target(query_id).await?
        else {
            info!(query_id = query_id, "openGauss 未找到可取消的活动查询");
            return Ok(false);
        };
        ...
        if let Err(error) = cancel_token.cancel_query(NoTls).await {
            warn!(error = %Self::format_opengauss_error(&error), query_id, backend_pid, "openGauss cancel token 取消失败，准备回退到 pg_cancel_backend");
        } else {
            info!(
                query_id = query_id,
                backend_pid = backend_pid,
                "openGauss cancel token 已发送"
            );
        }

        let cancel_client =
            Self::create_client(&config, Arc::new(StdMutex::new(Vec::new()))).await?;
        let cancelled: bool = Self::query_one_logged(
            &cancel_client,
            "openGauss 取消查询 SQL",
        ...
        info!(
            ...
            backend_pid = backend_pid,
            cancelled = cancelled,
            "openGauss pg_cancel_backend 执行完成"
        );
