...
function getIconConfig(node: TreeNode) {
  ...
  if (type === 'leaf') {
    ...
    if (key.includes('-composite-types') || key.includes('-composite-type') || key.includes('-composite-field')) return { icon: 'ph:stack-simple-duotone', color: 'var(--icon-color-cyan)' }
